...
# This function filters the bank list for the max loan size to match the 
# customers max loan size. 
...
def run():
    ...
    file_path = Path("./Unsolved/data/daily_rate_sheet.csv")
   ...
   # print(f"bank_data: {bank_data}")
    ...
    # filter for possible banks to use for the loan
    possible_banks = filter_max_loan_size (loan_amount, bank_data)
    
    print(f"Some possible banks that could managed this would would be - {possible_banks}")
